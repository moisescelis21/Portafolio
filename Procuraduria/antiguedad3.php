...
if (!isset($_SESSION["cedula"])) {
 ...
 if ($tiempo == "") {
  ...
  if (result.isConfirmed) {
    window.location= 'antiguedad.php';
  }
})</script>";
 }elseif ($porcentaje == "") {
 	echo "<body><script>Swal.fire({
 ...
 $sql="INSERT INTO primantiguedad VALUES('id','$porcentaje','$tiempo')";
 $ejecutar=mysqli_query($con,"SET CHARACTER SET 'utf8'");
 $ejecutar=mysqli_query($con,$sql);
 }
 	if(!$ejecutar){
 		echo"Su Prima de Antiguedad no ha sido registrado correctamente.";
 	}else{
    echo "<body><script>Swal.fire({
  title: 'Felicidades',
  text: 'Su Prima de Antiguedad ha sido registrado correctamente.',
  icon: 'success',
  confirmButtonColor: '#3085d6',
  cancelButtonColor: '#d33',
  confirmButtonText: 'Entiendo'
