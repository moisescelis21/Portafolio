...
const expresiones = {
	...
	nombre: /^[a-zA-ZÀ-ÿ\s]{1,2500}$/, // Letras y espacios, pueden llevar acentos.
    ...
    file: /^(?=.*\.(jpg|jpeg|png|gif))(?!\s*$).+/i, // Archivos
...
const validarFormulario = (e) => {
	switch (e.target.name) {
		case "titulo":
			...
			break;
        case "imagen":
            validarCampo(expresiones.file, e.target, 'imagen');
            break;
        case "redaccion":
            validarCampo(expresiones.nombre, e.target, 'redaccion');
            break;
        case "resumen":
            validarCampo(expresiones.nombre, e.target, 'resumen');
            break;
...
formulario.addEventListener('submit', (e) => {
	...
	if (campos.titulo && campos.imagen && campos.redaccion && campos.resumen) {
