...
?>
            <!-- METE ITEMS AQUI -->
          <div class="contenido">
            <form action="hijosedit2.php?id=<?php echo $id ?>" class="formulario" id="formulario" method="post" autocomplete="off" enctype="multipart/form-data">
			<!-- Grupo: Usuario -->
            <?php 
                        $conexion=mysqli_connect('localhost','root','','procuraduria');
                        $sql="SELECT * FROM primhijos WHERE id=$id";
                        $result=mysqli_query($conexion,$sql);
            ...
            <div class="container-inputs" id="grupo__valor">
                    <label class="formulario__label" for="valor">Valor</label>
                    <div class="form__inputs">
                        <input type="text" name="valor" id="valor" class="valor"
                            placeholder="Valor(Bs.)" value="<?php echo $valor ?>">
                        <i class="icon-close-outline form__validacion-estado"></i>
                    </div>
                    <p class="form__validacion-error">Introduzca el Valor(Bs.).</p>
                </div>
<?php } ?>
...
modeSwitch.addEventListener("click" , () =>{
    ...
    <!-- Javascript -->
    <script src="js/validacionesFormularios/formularioprimahijosedit.js"></script>
