<?php
 ...
 while($mostrar=mysqli_fetch_array($result)){
	$cantactual = $mostrar['cantidad'];
 	...
 	if ($cantnueva < 0) {
 		echo "<script>alert('La cantidad no puede ser mayor a la cantidad actual'); window.location= 'menuQ1.php?id=$id';</script>";
 	}else{
 ...
 if(!$ejecutar){
 		echo"Hubo Algun Error";
 			...
 			echo "<script>alert('La cantidad fue quitada exitosamente'); window.location= 'menuV.php';</script>";
 			}
