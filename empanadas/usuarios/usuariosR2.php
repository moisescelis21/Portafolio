<?php
 ...
 if ($validacion2) {
 	if (mysqli_num_rows($validacion2)>0) {
 		...
 		$sql="INSERT INTO usuarios VALUES('id','$nombre','$apellido','$usuario',SHA('$password'),'$email','$privilegio')";
 		$ejecutar=mysqli_query($con,"SET CHARACTER SET 'utf8'");
 		$ejecutar=mysqli_query($con,$sql);
 		if(!$ejecutar){
 		echo"Hubo Algun Error";
 		}else{
  			echo "<script>alert('Su usuario ha sido registrado correctamente'); window.location= 'usuariosV.php';</script>";
 		}
 	}	
}else{
	echo"Hubo Algun Error";
