        ...
        <header class="header">
            ...
            <h1>Modificar Puntaje</h1>
              ...
              <?php 
                        $id = $_GET['id'];
                        $conexion=mysqli_connect('localhost','root','','procuraduria');
                        $sql="SELECT * FROM estadisticas WHERE id='$id'";
                        $result=mysqli_query($conexion,$sql);
                        ...
                        while($mostrar=mysqli_fetch_array($result)){
                            ...
                            $preguntas_anuladas = $mostrar['preguntas_anuladas'];

                            
                        ?>           
            <form action="puntajedit2.php?id=<?php echo $id ?>" class="formulario" id="formulario" method="post" autocomplete="off" enctype="multipart/form-data">
			<!-- Grupo: Usuario -->
           

            <div class="container-inputs" id="grupo__tiempo">
                    <label class="formulario__label" for="tiempo">Correctas</label>
                    <div class="form__inputs">
                        <input type="text" name="correctas" id="correctas" class="correctas"
                            placeholder="Ingrese la Pregunta" value="<?php echo $correctas ?>">
                        <i class="icon-close-outline form__validacion-estado"></i>
                    </div>
                    <p class="form__validacion-error">Introduzca los años de antiguedad.</p>
                </div>
                <div class="container-inputs" id="grupo__tiempo">
                    <label class="formulario__label" for="tiempo">Incorrectas</label>
                    <div class="form__inputs">
                        <input type="text" name="incorrectas" id="incorrectas" class="incorrectas"
                            placeholder="Ingrese la Pregunta" value="<?php echo $incorrectas ?>">
                        <i class="icon-close-outline form__validacion-estado"></i>
                    </div>
                    <p class="form__validacion-error">Introduzca los años de antiguedad.</p>
                </div>
